...
func TestInsertTypePlacesANestedTypeAtAnIndex(t *testing.T) {
	...
	// Yjs implementation decodes — so the placement has to happen at insert.
	src := New()
	cells := src.GetArray("cells")
...
func TestInsertTypeBuffersWhenArrayDetached(t *testing.T) {
	doc := New()
	...
	doc.Transact(func(txn *Transaction) {
		first := NewMapPrelim()
		first.Set(txn, "kind", "first")
		outer.PushType(txn, first)

		head := NewMapPrelim()
		head.Set(txn, "kind", "head")
		outer.InsertType(txn, 0, head) // buffered: outer is still detached
	})
	if got := outer.Len(); got != 0 {
		t.Fatalf("detached array has %d items, want 0 (InsertType must buffer)", got)
	}

	doc.Transact(func(txn *Transaction) { root.Set(txn, "cells", outer) })
	if got := outer.Len(); got != 2 {
		t.Fatalf("attached array has %d items, want 2", got)
	}
	first, _ := outer.Get(0).(*YMap)
	if first == nil {
		t.Fatal("item 0 is not a map")
	}
	if v, _ := first.Get("kind"); v != "head" {
		t.Fatalf("item 0 kind = %v, want head (buffered InsertType must replay in order)", v)
	}
}
